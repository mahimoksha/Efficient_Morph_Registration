...
class EfficientTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        # x = x.view(x.shape[0], x.shape[self.axial_dim_1] * x.shape[self.axial_dim_2] , x.shape[self.extra_dim]*x.shape[4])
...
class EfficientTransformer(nn.Module):
    def __init__(self, pretrain_img_size=224,
        ...
        self.patch_embed = PatchEmbed(
            patch_size=patch_size, in_chans=in_chans, embed_dim=token_dim,
        ...
        if self.ape:
            ...
            self.pos_embd = SinPositionalEncoding3D(token_dim).cuda()
        ...
        self.downsamples = PatchMerging(dim=token_dim, norm_layer=norm_layer, reduce_factor=2)
    ...
    def forward(self, x):
        ...
        if self.ape:
            ...
            x = x.flatten(2).transpose(1, 2)
        x = self.pos_drop(x)
        x = self.downsamples(x,Wh,Ww,Wt)#.view(x.shape[0],Wh//2,Ww//2,Wt//2,x.shape[2]*4).permute(0,4,1,2,3)
...
class EfficientMorph(nn.Module):
    def __init__(self, config):
        ...
        token_dim = config.embed_dim #32
        embed_dim = config.token_dim #8
        ...
        self.up2 = DecoderBlock(token_dim*2, token_dim, skip_channels=token_dim if if_transskip else 0, use_batchnorm=False)  # 384, 40, 40, 64
        self.up3 = DecoderBlock(token_dim, config.reg_head_chan, skip_channels=token_dim // 2 if if_convskip else 0, use_batchnorm=False)  # 384, 80, 80, 128
        self.c1 = Conv3dReLU(2, token_dim // 2, 3, 1, use_batchnorm=False)
    ...
    def forward(self, x):
        ...
        if self.if_transskip:
            f2 = out_feats[-1]#20,24,28,192
            f3 = out_feats[-2]#40,48,56,96
        else:
            f1 = None
            f2 = None
            f3 = None
        
        x = self.up2(out_feats[-1], f3)
        x = self.up3(x, f4)
